...
# Updated uploader file handler
# قرار بدید جای نسخه قبلی تابع handler در فایل mother_bot.py
import os
import time
import uuid
from datetime import datetime
...
def handle_uploader_files(message):
    """
    به‌روز شده:
      - ساختار upload_type و پرچم‌های whitelist/permanent/deletable تصحیح شده‌اند.
      - خطاها هنگام دانلود/ارسال مدیریت می‌شوند.
      - فایل‌های موقت حذف می‌شوند.
      - نگهداری یک مرجع واحد در uploader_file_map برای لینک خصوصی و لینک چنل.
    """
    user_id = message.from_user.id
    opts = getattr(data_store, "auto_exec_options", {}) or {}
    user_state = data_store.get_user_state(user_id) or {}
    delete_after = user_state.get("state") == "uploader_delete_file_upload"
    file_type = message.content_type

    # اگر تایید whitelist از کاربر گرفته نشده، اطلاعات pending را ذخیره و سوال می‌کنیم.
    ...
    if not sdata.get("whitelist_confirmed"):
        ...
        markup.add(types.KeyboardButton("🔙 بازگشت به اپلودر"))
        bot.send_message(user_id,
                         "آیا می‌خواهید این فایل لیست سفید شود؟\n(اگر لیست سفید شود، فقط اونر/ادمین‌ها یا کاربران اضافه‌شده به لیست سفید فایل قادر به دانلود خواهند بود.)",
                         reply_markup=markup)
        return

    # بعد از تایید whitelist دوباره سِیت را می‌گیریم تا تصمیم نهایی ببینیم
    sdata = (data_store.get_user_state(user_id).get("data") or {})

    # ایجاد پیام پیشرفت
    progress_msg = send_progress_message(user_id, "شروع پردازش فایل...", 5)

    # تعیین file_id و orig_name بسته به نوع فایل
    file_id = None
    orig_name = None
    try:
        ...
        update_progress_message(progress_msg, f"❌ خطا در خواندن پیام: {e}", 100)
        bot.send_message(user_id, f"خطا در پردازش فایل: {e}", reply_markup=get_back_menu())
        return

    # کانال اپلودری
    uploader_channel = data_store.uploader_channels[0] if getattr(data_store, "uploader_channels", None) else None
    if not uploader_channel:
        update_progress_message(progress_msg, "❌ چنل اپلودری ثبت نشده است.", 100)
        bot.send_message(user_id, "❗️ چنل اپلودری ثبت نشده است.", reply_markup=get_back_menu())
        return

    # helper: تولید شیء فایل پایه با پرچم‌ها
    def _make_base_file_obj(uuid_str, ftype, ch_link, start_link, fname,
                            original_name=None, auto_modified=False, upload_type="permanent"):
        """
        upload_type expected values (backward-compatible):
          - "deletable" or "delete"              -> deletable (temporary)
          - "deletable_whitelist"                -> deletable + whitelist
          - "permanent" or "normal"              -> permanent
          - "permanent_whitelist"                -> permanent + whitelist
          - can accept dict: {"kind":"deletable"|"permanent","whitelist":bool}
        """
        # normalize upload_type
        kind = "permanent"
        whitelist_flag = False
        if isinstance(upload_type, dict):
            kind = upload_type.get("kind", "permanent")
            whitelist_flag = bool(upload_type.get("whitelist", False))
        else:
            ut = (upload_type or "").lower()
            if ut.startswith("delete") or ut.startswith("delet"):
                kind = "deletable"
            if "whitelist" in ut:
                whitelist_flag = True

        permanent = (kind == "permanent")
        deletable = (kind == "deletable")
        ...
        obj = {
            ...
            "auto_modified": bool(auto_modified),
            "upload_type": upload_type,
            "permanent": permanent,
            "deletable": deletable,
            "whitelist": whitelist_flag,
            "whitelisted_users": [],  # همیشه لیست برای ثبات ساختار
            "uploaded_at": datetime.utcnow().isoformat()
        }
        return obj

    # انتخاب upload_type بر اساس delete_after و whitelist تصمیم گرفته‌شده
    user_whitelist = bool(sdata.get("whitelist", False))
    base_kind = "deletable" if delete_after else "permanent"
    if user_whitelist:
        upload_type_val = f"{base_kind}_whitelist"
    else:
        upload_type_val = base_kind

    # کوتاه‌سازی: ارسال مستقیم برای document بدون تغییرات و بدون حالت حذف
    if file_type == "document" and not (opts.get("rename") or opts.get("filter")) and not delete_after:
        ...
        try:
            ...
            update_progress_message(progress_msg, f"❌ خطا در ارسال به چنل اپلودری: {e}", 100)
            bot.send_message(user_id, f"خطا در ارسال به چنل اپلودری: {e}", reply_markup=get_back_menu())
            return
        ...
        priv_link = None
        # جستجوی وجود فایل قبلی بر اساس channel_link
        ...
        if not priv_link:
            ...
            file_obj = _make_base_file_obj(priv_uuid, "document", ch_link, priv_link, orig_name,
                                          original_name=None, auto_modified=False, upload_type=upload_type_val)
            # نگهداری یک مرجع واحد برای هر دو کلید
            data_store.uploader_file_map[priv_link] = file_obj
            data_store.uploader_file_map[ch_link] = file_obj
        else:
            # فقط اطمینان از نگاشت channel -> همان آبجکت
            data_store.uploader_file_map[ch_link] = data_store.uploader_file_map[priv_link]
            # به‌روزرسانی metadata در صورت نیاز
            data_store.uploader_file_map[priv_link].update({
                "upload_type": upload_type_val,
    ...
    # در صورت نیاز به دانلود و rename/filter برای documents
    if file_type == "document" and (opts.get("rename") or opts.get("filter")):
        ...
        try:
            ...
            update_progress_message(progress_msg, f"❌ خطا در دانلود: {e}", 100)
            bot.send_message(user_id, f"❌ خطا در دانلود فایل:\n{e}", reply_markup=get_back_menu())
            # cleanup if exists
            try:
        ...
        try:
            ...
            update_progress_message(progress_msg, f"❌ خطا در خواندن فایل دانلودشده: {e}", 100)
            bot.send_message(user_id, f"خطا در خواندن فایل دانلودشده: {e}", reply_markup=get_back_menu())
            try:
                os.remove(temp_path)
            except Exception:
                pass
        ...
        settings = get_auto_rename_settings(user_id) or {}
        kw = settings.get("keyword", "") or ""
        filter_word = settings.get("filter", "") or ""
        new_name = orig_name
        ...
        try:
            ...
            update_progress_message(progress_msg, f"❌ خطا در نوشتن فایل تغییر یافته: {e}", 100)
            bot.send_message(user_id, f"خطا در آماده‌سازی فایل: {e}", reply_markup=get_back_menu())
            try:
                os.remove(temp_path)
            except Exception:
                pass
        ...
        try:
            ...
            update_progress_message(progress_msg, f"❌ خطا در ارسال فایل تغییر یافته: {e}", 100)
            bot.send_message(user_id, f"خطا در ارسال فایل به چنل اپلودری: {e}", reply_markup=get_back_menu())
            # cleanup
            try:
        ...
        if not priv_link:
            priv_uuid = str(uuid.uuid4())
            priv_link = f"https://t.me/{BOT_USERNAME}?start=file_{priv_uuid}"
            file_obj = _make_base_file_obj(priv_uuid, "document", ch_link, priv_link, new_name,
                                          original_name=orig_name, auto_modified=True, upload_type=upload_type_val)
            data_store.uploader_file_map[priv_link] = file_obj
            data_store.uploader_file_map[ch_link] = file_obj
        else:
            # نگاشت channel -> موجود و بروزرسانی metadata
            data_store.uploader_file_map[ch_link] = data_store.uploader_file_map[priv_link]
    ...
    # برای دیگر انواع فایل‌ها یا حالت delete_after (یا document در حالت delete_after)
    ...
    try:
        if file_type == "document":
            ...
            # برای بقیه انواع، پارامتر اضافه نمی‌دهیم
            sent_msg = send_func(uploader_channel, file_id)
    except Exception as e:
        update_progress_message(progress_msg, f"❌ خطا در ارسال به چنل اپلودری: {e}", 100)
        bot.send_message(user_id, f"خطا در ارسال فایل: {e}", reply_markup=get_back_menu())
        return
    ...
    priv_uuid = str(uuid.uuid4())
    priv_link = f"https://t.me/{BOT_USERNAME}?start=file_{priv_uuid}"
    file_obj = _make_base_file_obj(priv_uuid, file_type, ch_link, priv_link, orig_name,
                                  original_name=None, auto_modified=False, upload_type=upload_type_val)
    data_store.uploader_file_map[priv_link] = file_obj
    data_store.uploader_file_map[ch_link] = file_obj
    data_store.save_data()
